...
?>      
            ...
            <div>
                ...
                <p>On a Windows system, there is a somewhat greater level of freedom when choosing the application to open an SSH connection with. The <span class="text-bold">Command Prompt</span>, <span class="text-bold">PuTTY</span>, and <span class="text-bold">PowerShell</span> applications all allow a user to establish such an SSH connection. Below is what each application looks like respectively:</p>
            ...
            <div>
                ...
                <p>Above is the FileZilla user interface. Unlike the terminal application earlier, FileZilla gives the user a pleasant user interface to work with. At the top of the FileZilla application, you may notice the <span class="text-bold">“Host”, “Username”, “Password”, and “Port”</span> fields. In order to establish a connection, we need to fill in the proper information into these fields. The host field is the server you are trying to connect to with the username attached at the front. In our case, this is very similar to what we typed to establish a terminal SSH connection: <span class="text-bold"><username>@banjo.rit.edu</span>. The username field is the username you are trying to connect to the server with. It is the same username from the host field. The password field is the password associated with the username in the username field. The last field is the port field. By default, when establishing an SSH connection, the SSH protocol uses TCP port 22. Without getting too much into unnecessary detail, remember that we must establish our SSH connection on port 22. At this point, the fields should look something like this:</p>
